...
describe('Correct Puzzle', () => {
  it('should validate the correct solution', async () => {
    // Parse solution - using . for empty/cross and S for star
    const solutionLines = [
      '. . . S . . S . . .',
      '. S . . . . . . S .',
      '. . . S . S . . . .',
    ...
    const solverStars = new Array(10).fill(0);
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
    ...
    console.log('Solver row stars:', solverStars);
    const solverComplete = solverStars.every(count => count === 2);
    console.log(`Solver complete: ${solverComplete ? '✓' : '❌'}`);
  });
});
